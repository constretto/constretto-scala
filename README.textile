...
Constretto is cross compiled for scala versions 2.8.0, 2.8.1, 2.9.0 and 2.9.1 and deployed to
the sonatype snapshot repository. So if you are using Maven, you can simply add constretto-scala as dependencies in your pom:
...
<pre>
	<dependencies>
		...
		<dependency>
			...
			<artifactId>constretto-scala_2.9.1</artifactId>
...
<pre>
	val constrettoRepo = "Sonatype Nexus Snapshots" at "https://oss.sonatype.org/content/repositories/snapshots"
...
<pre>
	object Bar {
	  implicit def barConverter(implicit intC:ScalaValueConverter[Int]) = ScalaValueConverter[Bar](_.split(":") match {
	    case Array(first, second) => Bar(first, intC.convert(second))
	  })
	}
	case class Bar(a:String, b:Int)
	
	val bar:Option[Bar] = constretto.get[Bar]("someBarKey") // compiles just fine and uses the barConverter to convert strings to Foos	
</pre>
